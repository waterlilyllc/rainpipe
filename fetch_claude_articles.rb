...
class ClaudeArticleFetcher
  GOOGLE_API_URL = 'https://www.googleapis.com/customsearch/v1'
  
  def fetch_latest_articles
    query = "Claude AI Anthropic 最新 2025"
    ...
    params = {
      ...
      q: query,
      num: 10,
      dateRestrict: 'd30', # 過去30日間
      lr: 'lang_ja'
    }
    uri.query = URI.encode_www_form(params)
    
    puts "🔍 Claude AIの最新記事を検索中..."
  ...
  def save_articles(articles)
    ...
    Dir.mkdir('./data/claude_articles') unless Dir.exist?('./data/claude_articles')
    
    filename = "./data/claude_articles/claude_articles_#{Time.now.strftime('%Y%m%d_%H%M%S')}.json"
    File.write(filename, JSON.pretty_generate(data))
    
    # 最新版も保存
    File.write('./data/claude_articles/latest.json', JSON.pretty_generate(data))
...
# 実行
if __FILE__ == $0
  fetcher = ClaudeArticleFetcher.new
