...
class GeminiArticleFetcher
  GOOGLE_API_URL = 'https://www.googleapis.com/customsearch/v1'
  
  def fetch_latest_articles
    query = "Gemini CLI Google AI 最新"
    ...
    params = {
      ...
      q: query,
      num: 10,
      dateRestrict: 'd30', # 過去30日間（CLIは新しいので期間を広げる）
      lr: 'lang_ja'
    }
    uri.query = URI.encode_www_form(params)
    
    puts "🔍 Gemini CLIの最新記事を検索中..."
  ...
  def save_articles(articles)
    ...
    Dir.mkdir('./data/gemini_articles') unless Dir.exist?('./data/gemini_articles')
    
    filename = "./data/gemini_articles/gemini_articles_#{Time.now.strftime('%Y%m%d_%H%M%S')}.json"
    File.write(filename, JSON.pretty_generate(data))
    
    # 最新版も保存
    File.write('./data/gemini_articles/latest.json', JSON.pretty_generate(data))
...
# 実行
if __FILE__ == $0
  fetcher = GeminiArticleFetcher.new
